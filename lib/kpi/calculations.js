...
export function calculateMetrics(aggregate, businessMetrics, avgSpeedToPricing, aaronOffers = 0) {
    if (!aggregate)
        return null;
    const metrics = {
        avgProfitPerDeal: 0,
...
export function calculateFourLevers(metrics, aggregate) {
    if (!metrics)
        return null;
    // Yield
    const yieldValue = metrics.textYield;
    const yieldStatus = getStatus(yieldValue, STATUS_THRESHOLDS.yield.green, STATUS_THRESHOLDS.yield.yellow, true);
    // Offer Coverage
    const coverageValue = metrics.hotLeadToOfferRate;
    const coverageStatus = getStatus(coverageValue, STATUS_THRESHOLDS.offerCoverage.green, STATUS_THRESHOLDS.offerCoverage.yellow, true);
    // Close Rate
    const closeValue = metrics.contractToCloseRate || 70; // Default if no data
    const closeStatus = getStatus(closeValue, STATUS_THRESHOLDS.closeRate.green, STATUS_THRESHOLDS.closeRate.yellow, true);
    return {
        yield: {
...
export function detectBottleneck(levers) {
    if (!levers)
        return null;
...
function createBottleneck(lever, levers) {
...
export function buildTeamScorecards(aggregate, metrics) {
    if (!aggregate)
        return [];
...
export function isTeamWinning(aggregate) {
    if (!aggregate)
        return false;
